...
public class ItemGroup extends BambooView {
    ...
    protected void init(AttributeSet attrs) {
        ...
        mSlots = getChildCount();
        mItems = new Item[mSlots];
        mEditItems = new Item[mSlots];
        mImageViews = new SimpleDraweeView[mSlots];
        mDeleteButtons = new View[mSlots];
        for (int i = 0, n = mSlots; i < n; ++i) {
    ...
    public void setItems(@NonNull Item[] items) {
        for (int i = 0, n = mSlots; i < n; ++i) {
            mItems[i] = i < items.length ? items[i] : null;
    ...
    public void onClick(View v) {
        int index = v.getId();
        if (index >= 0 && index < mSlots && mImageViews[index].isSelected()) {
            super.onClick(v);
        }
    }
}
